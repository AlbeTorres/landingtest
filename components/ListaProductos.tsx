...
if(context?.filtro.dep!==null && context?.filtro.dep?.sections.length===0){
 auxProductolist=[]
}else if(context?.filtro.dep!==null && context?.filtro.dep?.sections.length!==0){
  auxProductolist=productos.filter(producto=>{

    let bol=context?.filtro.dep?.sections.some(section=>section.id===producto.section.id)
  ...
  return (
    ...
    <div className="w-full">
      ...
      <div className=" grid grid-cols-2 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
        {auxProductolist.length!==0 ? auxProductolist?.map((producto) => (
